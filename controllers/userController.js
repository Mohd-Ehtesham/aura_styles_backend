    ...
    .then((hashedPassword) => {
      if (request.file) {
        return imagekit
          ...
          .then((uploadResponse) => ({ uploadResponse, hashedPassword }));
      }
      return { uploadResponse: null, hashedPassword };
    })
    .then(({ uploadResponse, hashedPassword }) => {
      ...
      const newUser = new Users({
        ...
        isVerified: request.body.isVerified || false,
...
function getLoggedUser(request, response) {
  const userId = request.user.id;
  Users.findById(userId)
    .then((user) => {
      if (!user) {
...
function deleteUser(request, response) {
  ...
  Users.findById(_id)
    .then((user) => {
      ...
      const deleteImagePromise = user.imageFileId
        ? imagekit.deleteFile(user.imageFileId)
        : Promise.resolve();

      return Promise.all([deleteImagePromise, Users.findByIdAndDelete(_id)]);
    })
    .then(([, deletedUser]) => {
      if (!deletedUser) {
        return response.status(400).send({
      ...
      return response.status(200).send({
        success: true,
        message: "User deleted successfully, and avatar removed from ImageKit",
        data: deletedUser,
      });
    })
    .catch((error) => {
